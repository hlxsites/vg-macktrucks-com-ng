...
export function decorateLinks(block) {
  [...block.querySelectorAll('a')]
    ...
    .forEach((link) => {
      ...
      const external = !url.host.match('macktrucksnigeria.com') && !url.host.match('.hlx.(page|live)') && !url.host.match('localhost');
      if (url.host.match('build.macktrucksnigeria.com') || url.pathname.endsWith('.pdf') || external) {
        link.target = '_blank';
      }
    });
}
